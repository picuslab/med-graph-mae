...
def accuracy(y_pred, y_true):
    ...
    topk_values, topk_indices = torch.topk(y_pred, k, dim=1)
    
    # Create a new tensor of zeros with the same dimension as y_pred
    y_pred_labels = torch.zeros_like(y_pred)
    
    # Set values corresponding to the top k indices to 1
    y_pred_labels.scatter_(1, topk_indices, 1)

    # Calculate the number of 1s in y_true
    num_ones = y_true.sum(dim=1)

     # Indices of cases where the top k values do not match 1s in y_true
    incorrect_indices = (y_pred_labels * y_true).eq(0).nonzero()

    # Penalty: set the values of top k that do not match 1s in y_true to 0
    y_pred_labels[incorrect_indices[:, 0], incorrect_indices[:, 1]] = 0

    # Calculate the number of correctly predicted 1s
    num_correct_ones = (y_pred_labels * y_true).sum(dim=1)
    
    # Calculate accuracy as the percentage of correctly predicted 1s relative to the number of 1s in y_true
    accuracy = num_correct_ones.sum().item() / num_ones.sum().item()
...
def build_args():
    ...
    parser.add_argument("--max_epoch", type=int, default=3340,
                        help="number of training epochs") #def mio:1000
    ...
    parser.add_argument("--num_dec_layers", type=int, default=1) #ablation study-- prima default = 1
    parser.add_argument("--num_remasking", type=int, default=3) #ablation study-- prima default = 3    
    ...
    parser.add_argument("--remask_rate", type=float, default=0.5) #ablation study-- prima default = 0.5
    ...
    parser.add_argument("--alpha_l", type=float, default=2) #def: 2
    parser.add_argument("--optimizer", type=str, default="adam")
    
    parser.add_argument("--max_epoch_f", type=int, default=3340) #def : 1000
    parser.add_argument("--lr_f", type=float, default=0.0001) #det: 0.01
    ...
    parser.add_argument("--lam", type=float, default=1.0) #ablation study -- prima 1.0
